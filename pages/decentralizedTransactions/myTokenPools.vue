  ...
  export default {
    ...
    methods: {
      ...
      async getTransactionPairList(){
        let result = await decentralizedTransactionsApi.getTransactionPairListForAddress({chainId: this.$enums.ChainIdOldEnum.BUSINESS_CHAIN, address: this.defaultWallet.address.toLowerCase()});
      ...
      async getStableBalanceOf(){
        try{
           ...
           console.log('----getStableBalanceOf', err)
        }
        return '0'
      },
